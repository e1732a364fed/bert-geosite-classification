...
def process_domain(output_dir, domain):
    ...
    save_error(
        output_dir,
        domain,
        error_msg if "error_msg" in locals() else f"HTTP {response.status_code}",
    )
...
def main(list_file, output_dir):
    ...
    with open(list_file, "r") as f:
        domains = [
            ...
            for line in f
            if line.strip()
            and line.strip() not in processed_domains
            and not line.strip().startswith("regexp:")
        ]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_domain = {
            executor.submit(process_domain, output_dir, domain): domain
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch HTTP responses for a list of domains."
    )
    parser.add_argument(
        "-l",
        "--list-file",
        default="china-list.txt",
        help="Path to the list file containing domains.",
    )
    args = parser.parse_args()

    output_dir = os.path.join(
        os.getcwd(), f"{os.path.basename(os.path.splitext(args.list_file)[0])}_out"
    )
